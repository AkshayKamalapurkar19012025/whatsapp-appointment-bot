  ...
  return createPortal(
    <div className="modal-overlay schedule-modal-overlay" onClick={onClose}>
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={view === 'manage' ? 'Time Off' : formEditingBlock ? 'Edit Time Off' : 'Add Time Off'}
      ...
      >
        ...
        <div className="schedule-modal-header">
          <h3 className="appointment-details-heading" style={{ margin: 0 }}>
            {view === 'manage' ? 'Time Off' : formEditingBlock ? 'Edit Time Off' : 'Add Time Off'}
          </h3>
          <p className="muted schedule-modal-subtitle">
            {view === 'manage' ? dateHeading : 'Mark when the doctor will be unavailable.'}
          </p>
        </div>
...
function TimeOffForm({
  ...
  const formValid = dateRangeValid && hoursValid && reasonValid
  ...
  // day repeated N times.
  const draft = dateRangeValid ? draftBlockEntry() : null
          ...
          {duration === 'specific_hours' && (
            <div className="schedule-field-grid">
              <div className="schedule-field-group">
                ...
                <TimeCombobox value={fromTime} onChange={setFromTime} durationMinutes={30} ariaLabel="Time off start time" />
              </div>
              <div className="schedule-field-group">
                <span className="field-label">Until</span>
                <TimeCombobox value={untilTime} onChange={setUntilTime} durationMinutes={30} ariaLabel="Time off end time" />
              </div>
            </div>
          )}
