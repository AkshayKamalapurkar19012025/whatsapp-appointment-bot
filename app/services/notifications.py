...
Scope: WEB (patient) actions only. app/api/booking.py's WhatsApp flow
already gives real-time confirmation via its own conversational reply
in the same chat -- it is deliberately NOT wired to this module, since
sending an *additional* "confirmation" for an action the patient just
watched happen live would be a pointless duplicate, not a fix for
anything. A web-originated booking/cancel/reschedule has no such live
channel to confirm in, which is exactly the gap this phase closes (see
frontend/src/BookingFlow.tsx's WEB P3-era placeholder text about a
confirmation SMS "implemented in a later phase", updated this phase to
match).
